<?php
/**
 * CloudPe CMP Shared Console Page
 *
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header {
            background: #1a1a2e; border-bottom: 1px solid #2a2a4a; padding: 12px 20px;
            display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;
        }
        .header-left { display: flex; align-items: center; gap: 16px; }
        .logo { font-size: 18px; font-weight: 600; color: #4a9fff; }
        .vm-name { font-size: 16px; font-weight: 500; color: #fff; }
        .status-badge { display: inline-block; padding: 4px 10px; border-radius: 4px; font-size: 12px; font-weight: 600; text-transform: uppercase; }
        .status-active { background: #166534; color: #86efac; }
        .header-right { display: flex; align-items: center; gap: 12px; }
        .expires { font-size: 13px; color: #9ca3af; }
        .btn {
            display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 6px;
            font-size: 13px; font-weight: 500; cursor: pointer; border: none; transition: all 0.15s ease;
        }
        .btn-primary { background: #3b82f6; color: white; }
        .btn-primary:hover { background: #2563eb; }
        .btn-secondary { background: #374151; color: #e5e7eb; }
        .btn-secondary:hover { background: #4b5563; }
        .console-container { flex: 1; display: flex; flex-direction: column; background: #000; position: relative; }
        .console-frame { flex: 1; width: 100%; border: none; background: #000; }
        .loading-container, .error-container {
            flex: 1; display: flex; flex-direction: column; justify-content: center;
            align-items: center; padding: 40px; text-align: center;
        }
        .spinner {
            width: 48px; height: 48px; border: 4px solid #374151; border-top-color: #3b82f6;
            border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 20px;
        }
        @keyframes spin { to { transform: rotate(360deg); } }
        .loading-text { font-size: 16px; color: #9ca3af; }
        .error-icon {
            width: 64px; height: 64px; border-radius: 50%; background: #7f1d1d;
            display: flex; align-items: center; justify-content: center; margin-bottom: 20px; font-size: 32px;
        }
        .error-title { font-size: 20px; font-weight: 600; color: #fca5a5; margin-bottom: 10px; }
        .error-message { font-size: 14px; color: #9ca3af; max-width: 400px; line-height: 1.5; }
        .footer {
            background: #1a1a2e; border-top: 1px solid #2a2a4a; padding: 10px 20px;
            font-size: 12px; color: #6b7280; display: flex; justify-content: space-between; flex-shrink: 0;
        }
        .footer-hint { display: flex; gap: 16px; }
        .kbd { display: inline-block; padding: 2px 6px; background: #374151; border-radius: 3px; font-family: monospace; font-size: 11px; }
        .hidden { display: none !important; }
        .console-container:fullscreen { background: #000; }
        .console-container:fullscreen .console-frame { width: 100vw; height: 100vh; }
    </style>
</head>
<body>
    <div class="header">
        ...
        <div class="header-right">
            ...
            <button id="btn-fullscreen" class="btn btn-secondary hidden" onclick="toggleFullscreen()">Fullscreen</button>
            <button id="btn-reload" class="btn btn-primary hidden" onclick="loadConsole()">Reconnect</button>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadConsole() {
            if (!token) { showError('TOKEN_NOT_FOUND'); return; }
            ...
            try {
                ...
                showError('API_ERROR', 'Failed to connect to the console service.');
            }
        }
